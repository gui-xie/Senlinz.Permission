...
2. Add `P/permission.json` to the consuming project.
...
For direct package references, `Senlinz.Permissions` automatically includes `$(SenlinzPermissionFolder)/**/*.json` in `AdditionalFiles`, so files placed under `P/` are picked up without extra project configuration.
